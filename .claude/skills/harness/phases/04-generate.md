...
> ⚠️ **절대 금지:** Phase 4 완료 시 "개발 서버를 실행해보세요" 등의 안내 메시지를 출력하고 워크플로우를 종료하지 마세요.
> 빌드 성공은 Phase 5 진입의 전제 조건이지 종료 조건이 아닙니다.
> Phase 4가 완료되면 반드시 아래 안내를 출력하고 사용자 응답을 기다리세요.

구현 완료 후 사용자에게 아래 형식으로 결과를 보여주고 승인을 요청한다:

```
✅ Phase 4 완료 — 모든 Wave 구현 및 리뷰 통과

구현된 태스크: {완료된 태스크 목록}
빌드 상태: {PASS / FAIL}

→ 다음 단계: Phase 5 (테스트)
  단위 테스트 및 UI 스모크 테스트를 실행합니다.

Phase 5를 시작할까요? (Y/n)
```

사용자가 Y 또는 Enter를 입력하면 `generate.md`의 frontmatter `status`를 `approved`로, `timestamp`를 현재 시각으로 갱신한 후 Phase 5로 진행한다.
