...
### Phase 1: Clarify
요구사항을 명확히 한다. 구현 목적, 제약 조건, 성공 기준을 파악할 때까지 사용자와 대화한다. 요청이 너무 크면 서브태스크 분해를 제안하고, 접근법이 여러 갈래면 선택지를 제시한다. **프로젝트 유형**(정적 콘텐츠 / 비즈니스 로직 / API 서비스 / 혼합)을 파악하여 Phase 5 테스트 전략을 결정한다. 결과는 `clarify.md`로 저장하고 다음 페이즈에 인계한다.

### Phase 2: Context Gather
코드베이스를 탐색한다. 서브에이전트를 별도로 디스패치하여 메인 컨텍스트 창을 보호한다. 프로젝트 구조, 코드 컨벤션, 관련 기존 파일을 파악하여 `context.md`로 저장한다. 반복 실행 시 `.claude/.harness/context.md` 캐시를 재사용하여 불필요한 탐색을 줄인다. **LESSONS_LEARNED.md**가 존재하면 과거 실패 패턴을 서브에이전트 프롬프트에 주입한다. **context7 MCP**가 설치되어 있으면 주요 라이브러리의 최신 문서를 pull하여 훈련 데이터 기반의 낡은 패턴 사용을 방지한다.
...
- **Spec 리뷰** — 별도 서브에이전트가 요건 준수, **기능 완성도 체크리스트**(에러 상태 UI, 반응형, a11y, HTTP 상태코드 등) 검토. 이슈 발견 시 수정 후 재검토
- **코드 품질 리뷰** — 별도 서브에이전트가 코드 품질 및 **추가 완성도**(엣지 케이스, empty state, stub 주석 잔존 여부 등) 검토. Critical/Important 이슈는 수정 후 재검토

구현 서브에이전트(Implementer)는 **Definition of Done** 기준(콘텐츠 필드 충실도, 엣지 케이스 처리, 에러 경계 원칙)을 자체 점검 후 보고한다.

**사용자 승인 게이트**: 구현 결과 검토 후 승인해야 테스트로 진행한다. Phase 4 완료 시 워크플로우를 종료하지 않고 반드시 승인 안내를 출력한 후 대기한다.

### Phase 5: Test
구현된 코드를 테스트한다. **프로젝트 유형**에 따라 테스트 전략을 분기한다 — 정적 콘텐츠는 Playwright UI 스모크 테스트, 비즈니스 로직은 단위 테스트, API 서비스는 통합 테스트 우선. UI 파일 변경이 있고 Playwright가 없으면 Python/Node.js/Java 환경별 설치 가이드를 안내한다. 개발 서버를 직접 기동한 경우 **PID를 handoff에 기록**하여 Phase 6에서 종료 관리를 할 수 있게 한다. 테스트 실패 시 Phase 4로 피드백하여 재구현을 요청한다.

### Phase 6: Evaluate
Clarify에서 정의한 성공 기준 전체를 검증한다. 기준별 PASS/FAIL을 판정하고, 미달 항목은 원인에 따라 Phase 4 또는 Phase 5로 피드백한다. **Phase 5에서 기동한 개발 서버**가 있으면 PID를 확인하여 종료 여부를 안내한다. 전체 PASS 후 Phase 7로 진행한다.
...
Phase 7 커밋 후 사용자 승인 시 실행한다. 이번 실행의 반복 실패 패턴, 효과적이었던 패턴, 프로젝트 고유 주의사항을 분석하여 `memory.md`에 누적한다. FAIL 항목이나 재실행이 있었던 경우 **`LESSONS_LEARNED.md`**에 재발 방지 패턴을 기록한다 (git 추적 대상, 팀 누적 학습 자산). `memory.md`가 80줄을 초과하면 중복 제거 및 아카이브 고도화를 제안한다. 다음 실행에서 Phase 2와 Phase 3이 이 기억을 참고하여 같은 실수를 반복하지 않도록 한다.
...
실행 시작 시 하네스가 자동 생성할 파일 목록(`clarify.md`, `handoff-*.md`, `context.md`, `plan.md`, `tasks.json`, `generate.md`, `test.md`, `evaluate.md`, `document.md`)을 한 번에 안내하고 일괄 승인을 요청한다. 미리 승인하면 이후 각 파일 생성 시 별도로 확인하지 않는다.
...
### memory.md + LESSONS_LEARNED.md를 활용한 점진적 개선

Phase 8(Retrospect)을 꾸준히 실행하면 `memory.md`에 프로젝트별 패턴이 누적된다. Phase 2와 Phase 3에서 이 기억을 참고하여 이전에 실패했던 접근법을 반복하지 않는다.

FAIL이나 재실행이 있었던 경우엔 `LESSONS_LEARNED.md`에 재발 방지 패턴이 추가된다. Phase 2 서브에이전트는 실행 시점에 이 파일을 읽어 관련 주의사항을 프롬프트에 자동 주입한다. `memory.md`가 80줄을 초과하면 Phase 8에서 고도화(중복 제거, 오래된 항목 아카이브, 범용 패턴 승격)를 제안한다.
